...
srcIn=[symbol_t_any symbol_t_dup(1:polyphase+1)];
...
file_name = '../data/temp/dft_src12.dat'; 
fid = fopen(file_name, 'wt+');  
for i=1:length(srcIn)-1
    [srcOut_t,srcMemoryRx] = srcLinearUpSTx(srcIn(i),srcIn(i+1),cnstDelta,os,polyphase,polyfilter_lut,srcMemoryRx,fid);
...
%%dump data after pre-processing
% if data_len == 12
%   file_name = '../data/output/dft_preproc12.dat'; 
%   fid = fopen(file_name, 'wt+');
%   for k = 1:fft_len
%     fprintf(fid, '%15.8f       %15.8f\n', real(symbol_t_2n(k)), imag(symbol_t_2n(k)));
%   end
%   fclose(fid);  
% end
...
%%dump data after fft
% if data_len == 12
%   file_name = '../data/output/dft_fft12.dat'; 
%   fid = fopen(file_name, 'wt+');
%   for k = 1:fft_len
%     fprintf(fid, '%15.8f       %15.8f\n', real(symbol_f_2n(k)), imag(symbol_f_2n(k)));
%   end
%   fclose(fid);  
% end
