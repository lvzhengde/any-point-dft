function [srcOut,srcMemory] = srcLinearUpSTx(srcIn,srcIn_plus,cnstdelta,os,lengthi,lut,srcMemory,fid)
...
while srcMemory.acc<1 
    ...
    for ii=1:lengthi
        ...
        if (posCoefInt < os-1)
            c1(ii) = lut((ii-1)*os+posCoefInt+1);
            c2(ii) = lut((ii-1)*os+posCoefInt+1+1);
...
%     end
    ...
    %dump temporary results for debug
%     fprintf(fid, 'ampAcc = %15.8f\n', ampAcc);
%     fprintf(fid, 'posCoefInt = %d\n', posCoefInt);
%     fprintf(fid, 'posCoefDec = %15.8f\n', posCoefDec);
%     fprintf(fid, 'c1\n');
%     fprintf(fid, '%15.8f  ', c1);
%     fprintf(fid, '\nc2\n');
%     fprintf(fid, '%15.8f  ', c2); 
%     fprintf(fid, '\nc\n');
%     fprintf(fid, '%15.8f  ', c);    
%     fprintf(fid, '\ndelayline data\n');
%     for k = 1: length(srcMemory.buffer)
%       fprintf(fid, '%15.8f  %15.8f\n  ', real(srcMemory.buffer(k)), imag(srcMemory.buffer(k))); 
%     end
%     fprintf(fid, 'tmp = %15.8f\n', tmp);
%     fprintf(fid, 'delta_out = %15.8f\n', delta_out);
%     fprintf(fid, 'tmp = %15.8f\n', tmp);
end
